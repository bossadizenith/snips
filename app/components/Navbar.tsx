...
export function Navbar() {
  const { slides, addSlide, theme, setTheme, duration, setDuration } = useSnipsStore()
    ...
    <nav className="h-16 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-black px-6 flex items-center justify-between sticky top-0 z-10">
      <div className="flex items-center gap-4">
        <div className="w-8 h-8 bg-black dark:bg-white rounded-lg flex items-center justify-center">
          <span className="text-white dark:text-black font-bold text-lg italic">s</span>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 px-3 py-1 bg-zinc-50 dark:bg-zinc-900 rounded-md border border-zinc-200 dark:border-zinc-800">
           ...
           />
           <span className="text-[10px] font-mono text-zinc-500 w-8">{duration}s</span>
        </div>
        <div className="w-px h-6 bg-zinc-200 dark:bg-zinc-800 mx-1" />
        <button className="flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium text-zinc-600 hover:text-black dark:text-zinc-400 dark:hover:text-white transition-colors">
          <Play size={16} />
          Preview
        </button>
        <button className="flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium bg-black text-white hover:bg-zinc-800 dark:bg-white dark:text-black dark:hover:bg-zinc-200 transition-all shadow-sm">
          <Download size={16} />
          Export
        </button>
      </div>
