import type { KeyedToken, KeyedTokensInfo, MagicMoveRenderOptions } from "@/utils/types";
...
export class MagicMoveRenderer {
  ...
  constructor(target: HTMLElement | string, options: MagicMoveRenderOptions = {}) {
    ...
    if (typeof target === "string") this.container = document.querySelector(target) as HTMLElement;
  ...
  render(step: KeyedTokensInfo): Promise<void> {
    ...
    this.mapDom.forEach((el) => {
      if (el.tagName !== "BR") leave.push(el);
    });
    ...
    leave.forEach((el, idx) => {
      ...
      if (this.options.stagger) el.style.setProperty("--smm-stagger", `${idx * this.options.stagger}ms`);
      else el.style.removeProperty("--smm-stagger");
