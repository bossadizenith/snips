...
const inputVariants = cva(
  `w-full bg-transparent text-inherit outline-none placeholder:text-gray-a8`,
  {
    variants: {
      size: {
        small: "text-sm",
...
const ComboboxInputContainer = React.forwardRef<
  HTMLDivElement,
...
>(({ className, variant, size = "medium", children, icon, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(inputContainerVariants({ variant, size, className }))}
      {...props}
    >
      {children}
...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.GroupLabel
    ref={ref}
    className={cn("px-2 py-1.5 text-xs text-gray-8", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Empty
    ref={ref}
    className={cn(
      "px-2 py-4 text-center text-sm text-gray-8 empty:hidden -mb-2",
      className,
    )}
    {...props}
  />
